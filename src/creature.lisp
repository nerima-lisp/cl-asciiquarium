...
(defstruct (creature (:constructor %make-creature)) "A moving, drawable thing with privately owned sprite source and prepared render data."
...
(defun creature-frames (creature)
  "Return a copy of CREATURE's animation frames."
  (%copy-creature-frames (creature-%frames creature)))

(defun (setf creature-frames) (frames creature)
  "Replace CREATURE's animation frames with a privately owned copy."
  (%set-creature-frames creature frames))

(defun creature-style (creature)
  "Return a copy of CREATURE's drawing style."
  (copy-tree (creature-%style creature)))

(defun (setf creature-style) (style creature)
  "Replace CREATURE's drawing style with a privately owned copy."
  (%set-creature-style creature style))

(defun creature-z (creature)
  "Return CREATURE's immutable paint order."
  (creature-%z creature))
...
%SPRITE-PROTOTYPE is an immutable, private prepared sprite used by static
factories."
  (let ((creature
        (%make-creature
          :kind
...
(declaim (inline creature-x creature-y creature-tick-animation))
...
(defun creature-art (creature)
  ...
  (let ((index (creature-frame-index creature)))
    (if (eq (creature-facing creature) :left) (copy-seq (aref (creature-%mirrored-frames creature) index))
      (copy-seq (aref (creature-%frames creature) index)))))
...
(defun creature-blit (screen creature)
  ...
  (let* ((index (creature-frame-index creature))
    ...
    (loop for offset from 0 below run-count by 4
