...
(defparameter *fish-sprite-prototypes* (make-hash-table :test #'eq)
  "Maps fish species entries to theme/color-keyed immutable sprite prototypes.")

(defun %make-fish-sprite-prototype (entry color theme)
  "Create an immutable prepared sprite owner for ENTRY in COLOR and THEME."
  (make-creature :kind :fish-sprite-prototype
                 :frames (list (copy-seq (fish-species-entry-art entry)))
                 :style (visual-style theme color)
                 :%trusted-frames-p t
                 :%trusted-style-p t
                 :x 0
                 :y 0
...
(defun %fish-sprite-prototype (entry color theme)
  "Return ENTRY's immutable prepared sprite for COLOR, THEME, and mode."
  (let* ((prototypes
           (or (gethash entry *fish-sprite-prototypes*)
               (setf (gethash entry *fish-sprite-prototypes*)
                     (make-hash-table :test #'eq))))
         (theme (parse-visual-theme theme))
         (theme-prototypes
           (or (gethash theme prototypes)
               (setf (gethash theme prototypes)
                     (make-hash-table :test #'eq))))
         (color-key (unless *monochrome* color)))
    (or (gethash color-key theme-prototypes)
        (setf (gethash color-key theme-prototypes)
              (%make-fish-sprite-prototype entry color theme)))))
    ...
    (multiple-value-bind (facing dx) (fish-facing-and-dx facing dx)
      ...
      (let ((color (random-species-color entry)))
        (make-creature :world world
                       :kind :fish
                       :%sprite-prototype
                       (%fish-sprite-prototype entry color (world-theme world))
                       :facing facing
                       :z 5
                       :policy :wrap
