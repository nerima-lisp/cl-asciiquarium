...
- `RUN (&key width height fish-count seed interval stream shark-enabled-p
  ...
  CLI flags. `THEME` selects the initial `:ABYSS`, `:CORAL`, or `:MOONLIGHT`
  palette.
...
- `WORLD-WIDTH`, `WORLD-HEIGHT`, `WORLD-TICK`, `WORLD-CREATURES`,
  ...
  `WORLD-SHARK-COOLDOWN`, `WORLD-GUEST-COOLDOWN`, `WORLD-THEME`,
  `WORLD-HUD-VISIBLE-P` -- accessors.
...
- `RENDER-FRAME (renderer world)` -- draw and return the diffed frame output,
  reusing validated sprite caches and dirty regions where possible.
- `SHUTDOWN-RENDERER (renderer)` -- stop the renderer-owned worker executor
  and release its reusable frame state.
