          ...
          (it "eventually spawns a shark and kills a fish placed in its path, given a fixed seed"
              (with-seeded-random-state (42)
                                        (let ((world (make-world :width 40 :height 10 :fish-count 0)))
                                          ...
                                          (let ((fish (make-fish world :species :dart :x 20 :y (min cl-asciiquarium::+fish-lane-top+ (1- (world-height world))) :dx 0))) (cl-asciiquarium::%add-world-creature world fish) (dotimes (i 200) (unless (member fish (cl-asciiquarium::world-%creatures world)) (return)) (world-advance world)) (expect (member fish (cl-asciiquarium::world-%creatures world)) :to-be-falsy))))))
